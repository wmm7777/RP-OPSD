...
> **译者注｜公式 (5) 的 $\mathrm{sg}$ 与第 119 行"教师始终在不带梯度下评估"**
>
> $\mathrm{sg}[\cdot]$（stop-gradient）把教师那一路 $r_{g,t}(v)$ 从计算图 detach 掉。反向 KL $D(r\|p)$ 的梯度对两参数各有方向：$\partial/\partial\theta$ = "把学生拉向教师"（要），$\partial/\partial\phi$ = "把教师拉向学生"（不要）。$\mathrm{sg}$ 把后者置零，**教师在此 loss 里是目标值、不是可学变量**——这是知识蒸馏里 `teacher_logits.detach()` 的同一道理，只是 RP-OPSD 的教师要在学生自采前缀上重新前向，所以更要显式 no_grad。
>
> **为什么"不带梯度下评估"（`torch.no_grad()` / inference mode），三层原因：**
>
> 1. **正确性**：教师是 EMA 副本，是固定参考目标。若带梯度，autograd 会算出"把教师朝学生方向移"的梯度；若真用这个梯度更新 $\phi$，目标分布跟着学生一起塌，变成 moving target，反馈循环崩。on-policy 设定下这点尤其致命——学生采的是它自己（可能很烂）的前缀，带梯度会把教师往这些烂前缀拉，目标先烂，学生再对齐烂目标，正反馈塌掉。$\mathrm{sg}$ 在 loss 层防住这一侧，EMA($\rho$=0.05) 在参数层再把教师变化压慢，双保险。
> 2. **显存/算力**：带梯度意味着教师前向要把全部中间激活存下来供反向传播。教师同尺寸（9B）+ 全词表 softmax（~15 万维）激活显存直接爆炸；`no_grad` 不建图、不存激活，显存只剩前向本身。
> 3. **Top-K 选择的梯度偏置**：第 127 行选教师 Top-K 支撑集 $\mathrm{TopK}(r)$，这个选择依赖教师 logits。若选择带梯度，argtopk 不可微、且梯度只在被选中 token 上流，造成**有偏梯度**。把教师一路 $\mathrm{sg}$ 掉，Top-K 选择就变成固定掩码，梯度无偏（详见公式 8 译者注）。
>
> **效果区别**：带 $\mathrm{sg}$ → 学生单向逼近固定教师，稳定收敛；不带 $\mathrm{sg}$ 且教师可训 → 师生互相靠近，目标塌成学生当前分布，degenerate。即便教师冻结（`requires_grad=False`）但不加 `no_grad`，数值上等价不更新教师（无参数更新），却仍白白存激活 → 大模型 OOM。所以"冻结 + no_grad"才同时满足正确与高效，缺一不可。
>
> 注意 RP-OPSD 的教师不是外部固定模型而是学生自己的 EMA，$\mathrm{sg}$ 只截断 loss 内的反向通路；EMA 更新（公式 6）走的是另一条独立的参数复制通路，与 $\mathrm{sg}$ 不冲突——$\mathrm{sg}$ 管"loss 不反传给教师"，EMA 管"教师参数缓慢跟随学生"，两者正交。
...
> **译者注｜公式 (8) 截断偏差的完整推导与 $-p+r$ 校正的本质**
>
> 这一段回答一个反直觉问题：**学生已经和教师分布一模一样（$p\equiv r$）、loss 标量已经是 0，模型为什么还会继续乱改分布？** 答案是截断求和破坏了反向 KL 在概率单纯形上的驻点性质，使最优点处**有效梯度不为零**。训练靠梯度更新，不是看 loss 数字。
>
> **① 反向 KL 的逐项导数自带 +1。** $D(p\|r)=\sum_{v}p_v\log(p_v/r_v)$，对任意 $p_i$ 求无约束偏导：
>
> $$\frac{\partial}{\partial p_i} = \log\frac{p_i}{r_i} + 1$$
>
> 在最优解 $p\equiv r$ 处，$\log(1)+1 = 1$，即**每个 token 的无约束偏导都是 +1**。
>
> **② 全词表为何无 bug：概率归一把均匀 +1 吸收掉。** $p$ 落在单纯形 $\sum_v p_v=1$ 上，构造拉格朗日 $\mathcal{J}=\sum_v p\log(p/r)+\lambda(1-\sum_v p_v)$，约束驻点要求所有 $g_i-\lambda=0$。全词表时所有 $g_i=1$ 相等 → $\lambda=1$ 统一抵消 → **有效梯度全 0，模型停住**。
>
> **③ Top-K 截断为何出 bug：拉格朗日 λ 矛盾。** 截断损失 $L_{\text{trunc}}=\sum_{v\in S}p\log(p/r)$，S 外 token 不参与求和。拉格朗日：
>
> - $i\in S$：$\log(p_i/r_i)+1-\lambda$，在 $p=r$ 处 $=1-\lambda$，令其为 0 → **$\lambda=1$**
> - $j\notin S$：损失无该项，$0-\lambda=-\lambda$，令其为 0 → **$\lambda=0$**
>
> **$\lambda$ 不能同时等于 1 和 0 → 矛盾**。即截断后在 $p=r$ 最优点处，单纯形上不存在驻点，残存虚假有效梯度，模型继续把 S 内概率往上顶、挤压 S 外。bug 不来自"选了哪些 token"，而来自"反向 KL 截断子集后 +1 常数失去均匀性，归一约束再也吸不掉"。
>
> **④ $-p+r$ 校正的本质：把截断和变成逐项 Bregman 散度之和。** 校正后 $d_K=\sum_{v\in S}[p\log\frac{p}{r}-p+r]$，对 $p_i(i\in S)$ 求导 $=\log(p_i/r_i)+1-1=\log(p_i/r_i)$，在 $p=r$ 处 $=0$。更深层地，$p\log\frac{p}{r}-p+r$ 恰好是凸函数 $\psi(p)=p\log p-p$ 的 **Bregman 散度** $D_\psi(p,r)=\psi(p)-\psi(r)-\psi'(r)(p-r)$，具有**逐项性质**：每个 $(p,r)$ 对各自在 $p=r$ 处值=0 且梯度=0，不依赖别的 token、不依赖归一化。所以取任意子集求和都不会引入残留梯度。校正项 $-p$ 导数 $-1$ 手动抵消原始 RKL 导数自带的 $+1$；$+r$ 是常数（导数 0），只把 loss 标量调成"最优点处=0"。
>
> **⑤ 数值验证**（$V=5, K=2$，教师 $r=[0.4,0.3,0.15,0.1,0.05]$，学生 $p=r$；有效梯度 = $\text{raw\_grad}-\text{mean}(\text{raw\_grad})$，即拉格朗日 $\lambda=\text{mean}$）：
>
> ```
> ① 全词表 RKL:
>   raw grad        = [1. 1. 1. 1. 1.]      lambda = +1.0000
>   有效梯度(g-lam) = [0. 0. 0. 0. 0.]      ||有效梯度|| = 0.0000   -> 停住
>
> ② top-K 截断 RKL(无校正):
>   raw grad        = [1. 1. 0. 0. 0.]      lambda = +0.4000  (S 要 1、S 外要 0,矛盾)
>   有效梯度(g-lam) = [ 0.6  0.6 -0.4 -0.4 -0.4]   ||有效梯度|| = 1.0954   -> 残留虚假推力,模型会动!
>
> ③ top-K + (-p+r) 校正(公式8):
>   raw grad        = [0. 0. 0. 0. 0.]      lambda = +0.0000
>   有效梯度(g-lam) = [0. 0. 0. 0. 0.]      ||有效梯度|| = 0.0000   -> 停住
> ```
>
> ② 明确展示 λ 矛盾：raw grad 在 S 上=1、S 外=0，投影到单纯形后得非零有效梯度 `[+0.6,+0.6,-0.4,-0.4,-0.4]`，即学生已等于教师时模型仍会把 Top-2 概率往上顶、挤压其余 token——loss=0 但梯度≠0，训练震荡/漂移的根因。加 $-p+r$（公式 8）后有效梯度归零，矛盾消失。
>
> **⑥ 校正的局限与 RP-OPSD 的兜底。** 校正只修了"最优点处有虚假梯度"这个数学缺陷，**没有恢复 S 外 token 的监督信号**：校正后 $d_K$ 只依赖 $\{p_v:v\in S\}$，S 外 $p_j$ 在损失里不出现（只有归一约束里才有）。极小解集 $=\{p:p_v=r_v,\ v\in S,\ \text{S 外任意(只受归一)}\}$，$p\equiv r$ 只是其中一个点 → **S 外欠定、可漂移**。RP-OPSD 不加尾桶（第 127 行明确"不添加单独的尾桶"），靠三件事兜底：(a) on-policy 在学生自己访问的状态上蒸馏，S 外漂移不发生在被监督的前缀上意义有限；(b) Top-100 已覆盖教师概率质量绝大部分（自然语言下一 token 概率高度集中头部），S 外总质量极小；(c) EMA 教师缓慢跟随，目标本身不剧烈漂。这与 Vision-OPD（Yuan et al. 2026）走的**另一条等价修复**——top-100 后追加第 101 维"尾桶"使 K+1 维构成闭分布（和=1），让全词表吸收论证直接成立——形成对照：同一 bug 的两种修法，RP-OPSD 用逐项 Bregman 校正（Fix B），Vision-OPD 用尾桶补闭分布（Fix A）。
