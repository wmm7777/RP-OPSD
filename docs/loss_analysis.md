...
## §12 训练被 SIGTERM 中断 + epoch1 完整分析（2026-09-06 更新）

### 12.1 ⚠️ 训练已被外部信号杀掉，非自然完成

tmux session `rp_opsd_v3_no_ema` 已不存在。日志末尾显示：

```
*** SIGTERM received at time=1788606156 on cpu 122 ***
[2026-09-05 19:02:36,343 E 3357521 3357521] logging.cc:474: *** SIGTERM received ...
```

时间线还原：
- **19:02:36** 训练进程收到 SIGTERM 被杀，最后完成 step 1055（epoch 1，70%）
- **19:03:17** idleprotect_g0~g7 八个保护 session 创建（晚于 SIGTERM 41 秒），接管了全部 8 张 H20

**根因判断**：不是 OOM 也不是代码异常（无 traceback，只有外部 SIGTERM）。最可能是 cron 的 idle_gpu_protect 在 19:02 的 10 分钟检测周期判定 GPU 空闲/僵尸进程后下发了 kill——但训练当时明明在满载跑（step 1054→1055 间隔 177s 正常），属于 idleprotect 误判杀了真训练（memory `feedback_idleprotect_check_before_launch` 记录过这个坑：filler 进程像真训练易被误判）。无论根因，现状是：**训练停在 step 1055，epoch 1 完成 70%，checkpoint 到 1050**。

> ⚠️ **重启前必读**：memory `feedback_idleprotect_check_before_launch` 要求——启动训练/推理前先 `tmux ls | grep idleprotect` 杀保护会话再起，否则新训练会被 cron 10 分钟回填的 idleprotect 再次杀掉。当前 8 个 idleprotect session 占着全部 GPU。

### 12.2 epoch1 vs epoch0：1 epoch 已收敛，第 2 epoch 边际收益递减

用 tbparse 拉取完整 1055 步数据，按 epoch 切分：

| 指标 | epoch0 (1-751) first→last | epoch1 (752-1055) first→last | epoch1 mean | 趋势 |
|---|---|---|---|---|
| `vopd_loss` | 0.122→0.0487 | 0.0479→0.0494 | **0.0460** | epoch0 降 60%，epoch1 **平到 0.046 不再降** |
| `raw_jsd_token_mean` | 0.122→0.0482 | 0.0475→0.0489 | 0.0455 | 同上，epoch1 无额外收益 |
| `mopd_reverse_kl_term` | 0.118→0.0304 | 0.0314→0.0336 | 0.0299 | epoch0 降 74%，epoch1 持平 |
| `mopd_bias_correction` | 0.0034→0.0177 | 0.0161→0.0153 | 0.0157 | epoch0 上升（正常残差），epoch1 **稳定 0.015 不扩** |
| `student_topk_mass` | 0.986→0.961 | 0.965→0.968 | 0.966 | epoch0 降 2.5pp，epoch1 反略升 |
| `teacher_topk_mass` | 0.989→0.979 | 0.981→0.983 | 0.981 | 冻结教师，微降是学生输出微变被动反映 |
| `rollout_corr/kl` | 0.192→0.131 | 0.131→0.122 | 0.136 | epoch1 略降，on-policy 假设稳定 |

**关键诊断**：epoch0 已经把 loss 从 0.122 压到 0.049，epoch1 全程在 0.040-0.054 窄带波动，均值 0.046——**第 2 epoch 没有带来实质 loss 下降**。结合 §11.2 的 plateau 判断，**1 个 epoch（751 步）已基本收敛，继续跑第 2 epoch 是边际收益递减**。

### 12.3 安全判据全部通过，无发散迹象

用 §9.5 的三条危险判据逐项核对全程 1055 步：

| 判据 | 阈值 | 全程实测 | 结论 |
|---|---|---|---|
| `mopd_bias_correction` > 0.05 | 触发即应回退 ckpt | **max=0.0191** at step 671，>0.05 步数=**0** | ✅ 远低于危险线 |
| `student_topk_mass` < 0.90 | 概率泄漏超 10% | **min=0.9574** at step 917，<0.90 步数=**0** | ✅ 学生始终在教师支撑域内 |
| `rollout_ppl` > 50 | rollout 生成低质量 token | epoch1 max=150（单步尖峰），mean=12.5 | ✅ 偶发离群非整体崩塌 |

对比 v5-27B 发散时的数值（bias 0.204 / student_topk 0.603 / rollout_ppl 915），v3-no-ema 全程都在安全水位 4-10 倍之外。**确认：训练健康，没有朝发散方向移动。**

### 12.4 reverse_kl 持续下降 + bias 稳定 = 真收敛（非假象）

每 50 步采样的趋势表显示一条干净的真收敛曲线：

```
step   reverse_kl   bias_correction   student_topk
 50      0.0746        0.0046          0.983
250      0.0509        0.0114          0.973
450      0.0376        0.0149          0.967
650      0.0321        0.0149          0.967
850      0.0298        0.0159          0.966
1050     0.0292        0.0158          0.966
```

reverse_kl 单调下降（0.075→0.029），bias_correction 稳定在 0.011-0.016 不发散，student_topk 稳定在 0.966-0.983。这正是 §Q10 诊断口诀里的"reverse_kl 下降 + bias 稳定 → **真收敛**"——与 v5 的"reverse_kl 下降 + bias 上升 → 假收敛"形成对比。bias 从 0.003 缓升到 0.016 是正常的蒸馏残差（学生学到了能学的部分，剩余 gap 稳定），不是发散信号。

### 12.5 grad_norm 尖峰全清单：12 次良性噪声

全程 `grad_norm > 20` 共 12 次，每次都在 1-2 步内回落：

```
step  12: 351.7   ← warmup 阶段，LR 刚上来，最大
step  14:  32.8   ← warmup 阶段
step  58:  54.2
step 195:  41.0
step 242:  24.8   ← §3.2 当初标记"需关注"的那次
step 287:  32.3
step 330:  21.1
step 424:  34.8
step 574:  20.4
step 736:  38.1   ← epoch0→1 切换附近
step 841:  22.6
step 911:  41.9   ← epoch1 最大
```

**没有任何一次尖峰后出现 loss 反弹或 bias 飙升**——全部是偶发极端 IS-ratio batch 引起的瞬时噪声，被 IS `clip(max=2.0)` 截断后不影响 loss。这是 v3-no-ema（base 9B + 冻结 base 9B 教师）的正常噪声模式，与 v5 的"grad 尖峰后持续恶化"性质完全不同。§3.2/§7.2 对 step 242 grad_norm=46.9 的"需关注是否发散"可以彻底盖棺定论为良性。

### 12.6 rollout 文本：无系统性退化，step 741 偶发尖峰已自愈

跑 `detect_rollout_degradation.py` 覆盖 step 1→1055，关键区间：

| step | word_rep | char_rep | gibberish | 备注 |
|---|---|---|---|---|
| 1-500 | 0.8-2.3% | 0-0.5% | 0-0.5% | epoch0 健康 |
| 741 | **4.3%** (33/768) | 0.7% | 0.7% | epoch0→1 切换附近偶发尖峰 |
| 761 | 1.0% | 0.3% | 0.4% | **立即自愈** |
| 800-1000 | 0.8-2.0% | 0.4-0.9% | 0.1-0.5% | epoch1 稳定 |
| 1055 | 2.0% | 0.8% | 1.3% | 末步，gibberish 略高但非崩塌 |

对比 v5-27B 发散三阶段（word_rep 9.8% / char_rep 6.1% / gibberish 4.3%），v3-no-ema 全程各项指标都是其 **1/3 ~ 1/5**，且 step 741 的尖峰在 20 步内自愈，没有 v5 那种"短语循环→字符崩塌→乱码"的单调加速退化链。**文本质量确认无系统性退化。**

### 12.7 结论与收尾建议

1. **训练健康但已被中断**：1055 步数据全部支持"真收敛、无退化"的诊断，但训练未自然完成 2 epoch。
2. **ckpt-1050（epoch1 70%）已可用**：epoch1 的 loss 均值 0.046 与 epoch0 末 0.049 持平，ckpt-1050 和 ckpt-750（epoch0 末）质量预计接近，可优先用 ckpt-1050 做摘要质量抽检。
3. **是否补跑剩余 447 步（~22h）**：从 loss 曲线看边际收益极小（epoch1 均值 0.046 vs epoch0 末 0.049），**建议不补跑**，直接用 ckpt-1050 收尾，把 GPU 让给 §9.6 规划的 v6（SFT 9B + JSD 0.5 + 27B 教师）主推实验——那个实验才是能真正提升摘要质量的方向（v3-no-ema 是 9B←9B 同尺寸自蒸馏，能力上限就是 base 9B 本身）。
4. **若要补跑**：重启前先杀全部 idleprotect session（见 §12.1），从 `global_step_1050` resume，跑完剩余 452 步到 step 1502。

---

## §13 SFT vs RP-OPSD 评测口径统一对比 + 优化方向（2026-09-06）

> 回答核心问题："RP-OPSD 比不上 SFT 吗？"——**是的，统一口径后确实比不上**。此前 v3-no-ema 报告的 4.679 与 SFT 报告的 4.064 不可直接比较，因为口径不同，口径统一后结论反转。

### 13.1 口径差异是此前对比失真的根因

| 报告 | "均分"口径 | 计算方式 | 数值 |
|------|-----------|---------|------|
| v3-no-ema 评测报告 | **4 维度均分** | (准确性+简洁性+完整性+格式)/4 | 4.679 |
| sft_gold_397b 评测报告 | **5 项均分** | (准确性+简洁性+完整性+格式+语种遵循度)/5 | 4.064 |
| sft_ori 评测报告 | **5 项均分** | 同上 | 3.998 |

语种遵循度是 0/1 二值制（非 1-5 分），SFT 报告把它当第 5 项纳入平均会系统性拉低 ~0.4 分（因为 SFT 语种遵循≈1.0，相当于给 4 个高分维度除以 5 而非除以 4）。v3-no-ema 报告只除 4，所以表面分高。**两者口径不可比。**

### 13.2 统一口径后的真实对比（4 维度均分）

| 实验 | 训练方式 | 准确性 | 简洁性 | 完整性 | 格式 | **4维均分** | badcase% |
|------|---------|--------|--------|--------|------|-----------|----------|
| sft_gold_397b ep1.0 | gold 数据 full SFT | 4.569 | 4.773 | 4.977 | 5.000 | **4.830** | 4.8% |
| sft_ori ep1.5(最优) | 原始数据 full SFT | 4.546 | 4.792 | 4.754 | 4.969 | **4.765** | 8.3% |
| v3-no-ema step300 | 9B←9B 自蒸馏 | 4.306 | 4.571 | 4.937 | 4.967 | **4.695** | 8.9% |
| v3-no-ema step150 | 同上 | 4.265 | 4.555 | 4.934 | 4.961 | **4.679** | 8.9% |

**结论**：统一口径后，v3-no-ema（4.695）< sft_ori（4.765）< sft_gold_397b（4.830）。RP-OPSD 确实比不上 SFT，差距集中在**准确性**维度（v3-no-ema 4.306 vs sft_gold 4.569，差 0.263），而完整性/格式/语种遵循三门接近满分，差距不大。

### 13.3 为什么 OPD 比不上 SFT：三个根因

**根因 A：9B←9B 自蒸馏无能力注入**
v3-no-ema 教师 = 学生同模型（EMA 副本），教师的能力上限就是 base 9B 本身。OPD 只能让学生更"尖锐化"（把已有的模糊分布 sharpen），不能引入新知识。SFT 用 GPT-5 级 gold summary 直接注入学生不具备的知识（正确的数值、正确的图标语义）。**蒸馏是对齐分布，SFT 是注入知识，两者天花板不同。**

**根因 B：badcase 是视觉理解错误，token 级蒸馏无法解决**
v3-no-ema badcase 73% 是 entity_error：数值识别错误（24.7万 vs 2.47万差一个数量级、4.5G 认成 5G、45万 vs 4.5万）、图标状态认错（关闭的蓝牙/定位/勿扰认成开启）、动作/对象错误（挖掘机"向卡车装载"实为"向岸边卸石"）。这些是**视觉编码器的感知能力上限**，不是语言模型 token 分布问题。OPD 蒸馏的是 token 级分布，无法教学生"看清图里的数字是 544 不是 344"。SFT 用正确 gold summary 直接告诉学生正确答案。

**根因 C：OPD 在 on-policy 上强化学生的错误**
on-policy 蒸馏 = 学生自己生成 rollout → 教师在学生生成的 support 上蒸馏。学生看错图 → rollout 里生成错误描述 → 教师（EMA=学生自己慢变版）若也认错 → 蒸馏强化错误。**铁证**：step150→step300 badcase 数量完全相同（76→76），OPD 训练 150 步**一个 badcase 都没修复**。SFT 用外部 gold 强制纠正这些错误。

### 13.4 优化方向排序

| 优先级 | 方向 | 做法 | 预期增益 | 成本 | 风险 |
|--------|------|------|---------|------|------|
| **1（首选）** | **SFT→OPD 两段式** | 先 SFT（gold_397b 数据）把学生拉到 4.83，再 OPD on-policy 精修 | 4.83→4.85+，改善简洁/格式一致性，不改善 entity_error | 低（复用已有 SFT ckpt） | 低，师生分布接近不发散 |
| **2** | **换 27B 外部教师 + JSD** | v5 用 27B 教师 RKL 发散（§9），改 JSD α=0.5；27B 看图能力 > 9B 能注入正确视觉理解 | 准确性维度可能提升，缩小 entity_error | 中（部署 27B + 验证质量） | 27B 真实质量存疑（memory: 27B 3.893 < 9B 4.034 方向反了，需先验） |
| **3** | **C-GRPO reward-based** | 用 v5 evaluator（GPT-5 2-agent）打分当 reward，绕过教师 logit | 针对性改善 badcase（reward 惩罚 entity_error） | 高（开发+训练） | GRPO 在 LFM2 SSM 上未验证 |
| **4** | **hybrid loss SFT+OPD** | gold SFT loss + on-policy OPD loss 加权同时训 | SFT 保知识下限，OPD 精修分布 | 中高（调权重） | 两个 loss 权衡复杂 |
| **5（治本）** | **视觉理解根因** | 更高分辨率输入 / OCR 辅助 / 更强视觉编码器 | 直击 entity_error 根源 | 极高（改模型架构） | 工程量大 |

### 13.5 推荐路线

**短期（本周）**：方向 1（SFT→OPD 两段式）。用 sft_gold_397b epoch1.0 ckpt（4.830）做 OPD warm-start，师生同图同指令，JSD α=0.5。此时师生分布已经接近（都是 SFT 模型），OPD 不发散，主要解决 SFT 的 exposure bias（off-policy 过拟合 gold 风格）。预期不改善 entity_error（视觉硬伤），但提升简洁/格式一致性到 4.85+。

**中期**：方向 2（27B 教师）。先验证 27B 在 flash_note 上的真实摘要质量（跑 50 条样本 MOS），确认 27B > 9B 后再开训。用 JSD α=0.5（非 RKL，§9 证明 RKL 跨尺寸发散）。27B 看图能力更强，可能注入正确的视觉理解，缩小 entity_error。

**长期/兜底**：方向 3（C-GRPO）。若白盒教师路线走不通（27B 质量不够/部署不稳），转 C-GRPO 用 evaluator 当 reward，从机制上绕过教师 logit 质量问题。memory 有完整落地方案（`project_cgrpo_swift_landing`）。

### 13.6 关键判据：OPD 的价值定位

OPD 在 flash_note 上不是"替代 SFT"而是"精修 SFT"。SFT 注入知识（解决 entity_error 靠 gold），OPD 对齐分布（解决 exposure bias）。**正确用法是 SFT 先行、OPD 后接，而非 OPD 独立从 base 起步**。v3-no-ema 从裸 base 起步是 OPD 比不上 SFT 的直接原因——它既没有 SFT 的知识注入，又受限于 9B←9B 自蒸馏的能力天花板。两段式（SFT→OPD）才能让 OPD 发挥其分布精修的特长，而非与 SFT 在知识注入上硬碰。

> **⚠️ §13 根因 A 被 §14 修正**：下面的 §14 引入 VAD/OPD-V/SA-OPD 三篇论文分析后，"9B←9B 自蒸馏无能力注入"的结论被推翻——VAD 用 9B frozen 自身教师 +6.07，证明问题不在自蒸馏天花板，而在特权信号选错（分辨率特权在摘要任务上无效）。§13 的两段式（SFT→OPD）降为保底方案，首选改为 VAD 擦除对比特权。详见 §14。

---

## §14 VAD/OPD-V/SA-OPD 三篇论文分析 + §13 修正（2026-09-06）

> 来源：`explore_tasks/image_OPD调研报告.md` §3/§4。三篇均有公开代码，已 clone 到 `/data4/wumeimei/flash_note/{SA-OPD,VAD_Multimodal_OPD,OPD-V}`，均为 verl 框架（与 RP-OPSD 同构）。

### 14.1 三篇核心机制

| 论文 | arXiv | 设计空间类别 | 教师配置 | 9B 增益 | 代码仓库 |
|------|-------|------------|---------|---------|---------|
| **VAD** | 2607.28590 | ③ 内容擦除对比 | frozen 自身（反事实归因） | Avg6 +6.07 | VAD_Multimodal_OPD |
| **OPD-V** | 2608.05131 | ② 区域裁剪特权 | detached copy（模态平衡） | 6-bm +7.88 | OPD-V |
| **SA-OPD** | 2608.03632 | ⑥ 教师放 prompt | 35A3B/9B 外部教师 | 2B +3.2 | SA-OPD |

**VAD（反事实视觉归因）**：擦除图中区域 → 对比教师/学生输出变化 → 分离"可归因于视觉的 token"→ 只在这些 token 上蒸馏，不蒸馏靠语言先验的 token。擦除后输出不变的 token = 语言先验驱动（虚假视觉信号），跳过；擦除后输出变化的 token = 真依赖视觉，强化蒸馏。

**OPD-V（模态平衡 trust region）**：区域裁剪特权（师看 bbox 裁剪图，生看全图）+ 模态平衡 trust region——文本主导时限制更新幅度，防止视觉 token 的梯度被文本 token 淹没。步时 -24.7%、响应长度 -74.5%，效率优势明显。

**SA-OPD（虚假视觉信号过滤）**：识别"教师 token 被语言先验驱动而非视觉证据"，过滤这些虚假信号不蒸馏。通过"答案无关提示"让教师生成时不能靠先验，对比出真正依赖视觉的 token。

### 14.2 对 flash_note 的匹配度分析

**flash_note 的核心痛点**（§13 诊断）：
- 73% badcase 是 entity_error（数值/图标/动作认错）= 视觉感知硬伤
- v3-no-ema 完整性/格式接近满分（~4.94/4.97）但准确性最低（4.306）= 视觉信号被文本梯度淹没
- step150→300 badcase 不变（76→76）= on-policy 蒸馏强化错误

三篇逐一匹配：

**VAD — 直接攻击 entity_error 根因**：
- entity_error（24.7万→2.47万、4.5G→5G、图标状态认错）的根源正是"模型靠文本先验生成而非看图"。VAD 的擦除对比能定位"哪些 token 依赖视觉"（擦掉数字区域后数值 token 变化 = 视觉归因 token），只在这些 token 上强化蒸馏 → 直接攻击 entity_error。
- **关键**：VAD 是 9B frozen 自身教师（和 v3-no-ema 同范式），但 +6.07，证明 9B 自蒸馏天花板不低——问题在特权信号。v3-no-ema 用"分辨率特权"（师看高清），但通知摘要不需要高分辨率细节，特权挖不出未利用的视觉证据；VAD 用"擦除对比"直接定位视觉依赖 token，有效。

**OPD-V — 解决"文本主导淹没视觉信号"**：
- flash_note 摘要是文本主导任务，v3-no-ema 现象（完整性/格式满分但准确性低）正是"视觉 token 梯度被文本梯度淹没"的典型。OPD-V 的模态平衡 trust region 保护视觉 token 学习信号。
- OPD-V 和 RP-OPSD 同类（都区域裁剪特权）但 +7.88 远超 RP-OPSD 增益，差异就在 trust region——这解释了为什么你的裸裁剪特权比不上加了 trust region 的方法。

**SA-OPD — 配合 27B 教师路线**：
- 命中 flash_note drift 根因（教师盲图靠文本先验）。27B 教师可能靠"通知类摘要常出现 5G"的先验生成"4.5G→5G"而非看图，SA-OPD 过滤这种虚假信号。
- 但 SA-OPD 是外部教师范式（2B←9B），9B 自蒸馏路线不直接适用。需走 27B→9B 路线才用得上。

### 14.3 §13 修正

**§13.3 根因 A 修正**：
- 原结论："9B←9B 自蒸馏无能力注入，OPD 只能 sharpen 已有分布，不能引入新知识"——**被 VAD 推翻**。
- 修正结论：9B←9B 自蒸馏**可以注入视觉感知能力**，前提是特权信号能挖出学生未利用的视觉证据。VAD 的擦除对比在 9B frozen 自身上 +6.07 = 铁证。v3-no-ema 失败的根因不是"自蒸馏天花板低"，而是"分辨率特权在摘要任务上无效"——通知摘要不需要高分辨率细节，分辨率特权挖不出视觉证据；擦除对比能。

**§13.4 优化方向修正**：

| 优先级 | 方向 | 攻击点 | 依据 | 对 §13 原方向的调整 |
|--------|------|--------|------|-------------------|
| **1（新首选）** | **VAD 擦除对比** | entity_error 根因 | 9B 自蒸馏同范式 +6.07，零外部依赖 | 取代原 §13 方向1（SFT→OPD 两段式） |
| **2** | **OPD-V 模态平衡 trust region** | 视觉信号被文本淹没 | 9B +7.88，同裁剪特权+trust region | 新增，可叠加到 VAD 上 |
| 3 | SFT→OPD 两段式（保底） | exposure bias | §13 原方向1 | 降为保底 |
| 4 | 27B+JSD+SA-OPD 虚假信号过滤 | 教师盲图 drift | SA-OPD 补强 27B 路线 | SA-OPD 新增强 |
| 5 | C-GRPO（兜底） | reward 约束 | §13 原方向3 | 不变 |

**修正后的核心判断**：OPD 在 flash_note 上比不上 SFT，不是因为"9B 自蒸馏无能力注入"（VAD 证伪），而是因为**特权信号选错 + 缺少视觉信号保护**。换 VAD 擦除对比特权 + OPD-V trust region，9B 自蒸馏即可大幅提升准确性，不必依赖 27B 教师或 SFT warm-start。这是比 §13 两段式更治本的路线。
