...
def _load_mme_realworld_parquet(repo_id, out_dir, img_subdir):
    ...
    # MME-RealWorld is split across tens of gigabytes of parquet shards. Loading
    # the complete binary image column into one Arrow table can exceed the
    # preparation host's memory, so preserve the same row schema while yielding
    # small record batches to the existing image extraction loop.
    def iter_rows():
        scanner = dataset.scanner(
            columns=["bytes", "index", "question", "multi-choice options", "answer"],
            batch_size=16,
            use_threads=False,
        )
...
def _prepare_mme_realworld_shard(shard_path, img_dir, part_path, lang="en"):
    """Convert one MME-RealWorld parquet shard in an isolated process."""
    import pyarrow.parquet as pq

    img_dir.mkdir(parents=True, exist_ok=True)
    parquet = pq.ParquetFile(str(shard_path), memory_map=False)
    rows = (
        row
    ...
    data = _process_mme_realworld_rows(rows, img_dir, lang=lang)
    tmp_path = part_path.with_suffix(part_path.suffix + ".tmp")
    with open(tmp_path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False)
    os.replace(tmp_path, part_path)
    print(f"Generated shard part: {part_path} (records={len(data)})")


def _merge_json_array_parts(part_paths, out_json):
...
def _prepare_mme_realworld_resumable(repo_id, out_dir, img_subdir, out_json, benchmark, lang="en"):
    """Prepare large MME datasets shard-by-shard so Arrow memory cannot accumulate."""
    local_dir = out_dir / repo_id.replace("/", "_")
    snapshot_download(repo_id, repo_type="dataset", local_dir=str(local_dir))
    data_dir = local_dir / "data"
    ...
    img_dir = out_dir / img_subdir
    part_dir = out_dir / f".{out_json.stem}_parts"
    part_dir.mkdir(parents=True, exist_ok=True)
    part_paths = []
    for shard_index, shard_path in enumerate(shards):
        part_path = part_dir / f"{shard_index:05d}_{shard_path.stem}.json"
        part_paths.append(part_path)
        if part_path.exists():
            print(f"Reusing shard part [{shard_index + 1}/{len(shards)}]: {part_path}")
            continue
        print(f"Preparing shard [{shard_index + 1}/{len(shards)}]: {shard_path.name}")
        subprocess.run(
            [
                sys.executable,
    ...
    count = _merge_json_array_parts(part_paths, out_json)
    print(f"Generated: {out_json} (records={count}, shards={len(shards)})")
...
def main():
    ...
    parser.add_argument("--mme-shard", default=None, type=Path, help=argparse.SUPPRESS)
    parser.add_argument("--mme-image-dir", default=None, type=Path, help=argparse.SUPPRESS)
    parser.add_argument("--mme-part", default=None, type=Path, help=argparse.SUPPRESS)
    parser.add_argument("--mme-lang", default="en", choices=["en", "cn"], help=argparse.SUPPRESS)
    args = parser.parse_args()

    if args.mme_shard is not None:
        if args.mme_image_dir is None or args.mme_part is None:
            parser.error("--mme-image-dir and --mme-part are required with --mme-shard")
        _prepare_mme_realworld_shard(
            args.mme_shard,
            args.mme_image_dir,
            args.mme_part,
            lang=args.mme_lang,
    ...
    if benchmark == "zoombench":
        ...
        _prepare_mme_realworld_resumable(
            "yifanzhang114/MME-RealWorld-Lmms-eval",
            out_dir,
            "MME_RealWorld_Full_images",
            out_json,
            benchmark,
            lang="en",
        )
