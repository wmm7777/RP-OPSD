#!/bin/bash
# 机器3 部署 Qwen3.5-397B-A17B-FP8（多模态 MoE，线性+full attention 混合，FP8）
...
# 机器3 系统 libstdc++ 旧（只到 CXXABI_1.3.13，缺 1.3.15），前置 env 新版 libstdc++.so.6.0.35（含 1.3.15/1.3.17）
export LD_LIBRARY_PATH=/data1/meimei.wu/miniforge3/envs/verl_opd_flashnote/lib:${LD_LIBRARY_PATH:-}

export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7
export TMPDIR=/dev/shm/vllm_m3            # 坑1：跨机勿用 sshfs 路径
mkdir -p "$TMPDIR"
...
LOG=/data4/wumeimei/flash_note/RP-OPSD/.runtime/flashnote_summary/deploy_397b_m3.log
mkdir -p "$(dirname "$LOG")"
