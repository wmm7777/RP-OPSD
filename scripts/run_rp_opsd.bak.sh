...
# =============================================================================
# RP-OPSD Training Script
# Paper: Resolution-Privileged On-Policy Self-Distillation for
...
# 必须满足 PPO_MAX_TOKEN_LEN_PER_GPU >= MAX_PROMPT_LENGTH + MAX_RESPONSE_LENGTH，
# 否则 use_dynamic_bsz=True 路径会在 prepare_dynamic_batch 触发
# AssertionError: max_token_len must be greater than the sequence length。
# 与 config/best.env 的 RP_OPSD_PPO_MAX_TOKEN_LEN_PER_GPU 保持一致。
...
EXPERIMENT_NAME="RP-OPSD-${MODEL_NAME}"
PROJECT_NAME="RP-OPSD"
TRAINER_DEFAULT_LOCAL_DIR="${RP_OPSD_LAUNCHER_CHECKPOINT_DIR:-${PROJECT_ROOT}/checkpoints/${EXPERIMENT_NAME}}"
TRAINER_ROLLOUT_DATA_DIR="${RP_OPSD_LAUNCHER_ROLLOUT_DIR:-${PROJECT_ROOT}/rollouts/${EXPERIMENT_NAME}}"
