#!/usr/bin/env bash
# =============================================================================
# RP-OPSD v2 副本启动脚本（机器2冗余备份）
# 与机器4主任务配置完全一致，仅改命名避免冲突：
#   - OUTPUT_DIR: flashnote_train_v2 -> flashnote_train_v2_rep
#   - EXPERIMENT_NAME: RP-OPSD-Qwen3.5-9B -> RP-OPSD-Qwen3.5-9B-REP
#   - tmux session: flashnote_train_v2 -> flashnote_train_v2_rep
# 长度口径 6k 与主任务一致。
# =============================================================================
...
OUTPUT_DIR="$PROJECT_ROOT/outputs/flashnote_train_v2_rep"
...
# ---------- 实验元数据（改名避免 tensorboard 冲突） ----------
EXPERIMENT_NAME="RP-OPSD-Qwen3.5-9B-REP"
...
export TMPDIR=/dev/shm/ray_h20_2_rep
export _RAY_TMPDIR=/dev/shm/ray_h20_2_rep
export RAY_TMPDIR=/dev/shm/ray_h20_2_rep
mkdir -p "$TMPDIR"
ulimit -c 0
...
cd "$PROJECT_ROOT"
echo "[run_rp_opsd_v2_rep] experiment=$EXPERIMENT_NAME"
echo "[run_rp_opsd_v2_rep] max_prompt=$MAX_PROMPT_LENGTH max_response=$MAX_RESPONSE_LENGTH max_model_len=$MAX_MODEL_LEN ppo_max_token_len_per_gpu=$PPO_MAX_TOKEN_LEN_PER_GPU"
echo "[run_rp_opsd_v2_rep] data=$TASK_TRAIN_FILE"
echo "[run_rp_opsd_v2_rep] output=$OUTPUT_DIR"
