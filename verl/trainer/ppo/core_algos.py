...
def compute_self_distillation_loss(
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        if use_topk:
            ...
            if distillation_objective == "mopd_topk_reverse_kl":
                if self_distillation_config.distillation_add_tail:
                    raise ValueError("mopd_topk_reverse_kl does not support a tail bucket.")

                # MOPD Eq. (5): sum_v p_s(v) log[p_s(v) / p_t(v)] - p_s(v) + p_t(v).
                ...
                diagnostic_tensors = {
                    ...
                    "self_distillation/mopd_reverse_kl_term_mean": reverse_kl_term.sum(-1),
                    "self_distillation/mopd_bias_correction_mean": bias_correction.sum(-1),
                }
        ...
        if not (use_topk and distillation_objective == "mopd_topk_reverse_kl"):
