...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        ...
        distillation_objective (str): Distributional objective. ``generalized_jsd`` preserves the
            original alpha-interpolated KL/JSD behavior; ``mopd_topk_reverse_kl`` implements the
            bias-corrected top-k reverse KL from MOPD.
        distillation_topk_source (str): Policy that chooses the top-k support. Options: ``student``
            (the original Vision-OPD behavior) or ``teacher`` (required by MOPD top-k).
    ...
    def __post_init__(self):
        ...
        valid_distillation_objectives = ["generalized_jsd", "mopd_topk_reverse_kl"]
        ...
        if self.distillation_objective == "mopd_topk_reverse_kl":
            if not self.full_logit_distillation or self.distillation_topk is None:
                raise ValueError(
                    "mopd_topk_reverse_kl requires full_logit_distillation=True and distillation_topk to be set."
                )
            if self.distillation_topk_source != "teacher":
                raise ValueError("mopd_topk_reverse_kl requires distillation_topk_source='teacher'.")
            if self.distillation_add_tail:
                raise ValueError("mopd_topk_reverse_kl requires distillation_add_tail=False.")
            if self.alpha != 1.0:
                raise ValueError("mopd_topk_reverse_kl requires alpha=1.0.")
